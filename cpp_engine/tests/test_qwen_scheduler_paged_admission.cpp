...
bool wait_for(const dsv4::QwenBatchScheduler& scheduler, Predicate pred,
...
void test_admission_is_bounded_by_blocks(const std::string& dir) {
    ...
    dsv4::QwenBatchScheduler scheduler(&engine, kSlots);
    ...
    require(wait_for(scheduler, [](const dsv4::QwenBatchScheduler::Stats& s) {
                return s.running_requests == 2 && s.waiting_requests == 1;
            }),
            "exactly two requests should be admitted, the third held on blocks");

    dsv4::QwenBatchScheduler::Stats stats = scheduler.get_stats();
...
void test_oversized_request_rejected(const std::string& dir) {
    ...
    dsv4::QwenBatchSamplingParams ok;
