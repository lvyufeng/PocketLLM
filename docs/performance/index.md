...
| [Ascend 910A single-request decode](ascend_single_request_tps.md) | What the rows=1 step's 104.3 ms is made of — 101.2 ms of it batch-independent, 62 ms of that 129 all-reduces — the 85 TPS HBM ceiling, MTP measured as a 2.2x loss, and the hand-written cross-process collective built from the probe's payload-carried barrier: worth 106.33 -> 77.32 ms, 9.41 -> 12.94 TPS at rows=1 over thirty-six interleaved pairs, shipped opt-in because the launcher's reproducibility gate is failed by both arms (3 of 36 HCCL, 8 of 36 hand-written) and does not separate them. |
