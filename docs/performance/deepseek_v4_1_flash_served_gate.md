...
| Request | `temperature 0.0`, `max_tokens 64`, streamed with `stream_options.include_usage`, filler prompt of 1364 or 32524 tokens |
| Client | `/tmp/v41_gate.py` — `prompt_tokens / ttft` for prefill and the inter-token cadence of the SSE stream for decode |
| Startup | 457.8 GiB of routed experts pinned in **99.3–102.6 s a rank**, all 48 shards loaded in **130.5–133.7 s**, four `POCKETLLM_RANK_READY` |
| Logs | `/tmp/v41_serve_short_fixed.log`, `/tmp/v41_serve_32k_fixed.log`, `/tmp/v41_serve_32k_sorted.log`; measurement output `/tmp/v41_gate_fixed_32k.txt`, `/tmp/v41_gate_32k_sorted.txt` |
...
Three differences make a served column not a launcher column, and none of them is the runtime's:
...
- **Decode is timed at the client.** The SSE cadence here is wall-clock between chunks arriving over
  ...
  against the launcher's graphed 229, and 220–225 ms at 1364 prompt tokens against its 201 — a
  consistent ~20–30 ms a step that is not a step.
...
- **Each request re-records its graphs**, since the adapter released them when the last one ended.
  That is 2.7 s of the first request's ttft at 1364 prompt tokens and is inside every leg of the
  table above; it is the price of serving one request at a time with graphs on, and it is not in the
  launcher's numbers at all.
...
- **It does not claim the served decode matches the launcher's.** At 32768 it is 3.82–3.95 against
  4.37 and at 1364 prompt tokens it is 4.45–4.53 against 4.98, and this page has not separated the
  deal, the client-side cadence and the per-request re-capture into a single number. The evidence it
  does have is that both decode legs are ~10% down and the short leg is the one with no deal in it.
- **The 262144 leg has not been served.** Its launcher row is 103.58 tok/s at 3.86 tok/s and it is the
  leg where the pool must be narrowed to 148, which is the same setting the 32768 legs above ran on.
