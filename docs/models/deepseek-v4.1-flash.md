...
**Inspect only: both config layouts the release ships are read into one schema, a 38-check safetensors-header audit — 39 against the release's own index — runs on the host and distinguishes a shard that has not downloaded from a shard that is wrong, the Engram hash front end that addresses the two 189 GiB tables is reproduced and tested, and the CSA2 attention stack — the three layer modes, the shared KV and index caches, and the two-level indexer — runs over random parameters; nothing loads a V4.1 weight and nothing generates tokens.**

Nothing in this page generates tokens. The attention half of a V4.1 layer is the one piece of layer-level code here: `src/models/deepseek_v4_1/attention.py` builds an `AttentionStack` over a config and runs a prefill or a decode step through it, but it is layers with no model around them — no embedding, no MoE, no Engram consumer, no hyper-connections — and every parameter in it is random, so it can be driven and measured and not compared to anything. `kv_source_layers` and `candidate_source_layer` are config key names *and*, since that module, values a forward pass reads. `ffn.gate.bias_vl` is still only the first kind: it is a real F32 `[384]` tensor on every backbone layer of the checkpoint, read out of the shards, but nothing here consumes it. `engram` does appear — `src/encoding/engram.py` reproduces the reference's tokenizer-side front end — but that is address arithmetic over a tokenizer, not a layer of the model. `src/models/deepseek_v4_1/kernels.py` is the op layer those attention layers call into, and `cpp_engine/engine/deepseek_v4_engine.cpp` and `src/models/deepseek_v4/` target the 43-layer, 4096-hidden DeepSeek-V4-Flash geometry.
...
- The audit has run against the real checkpoint at every stage of its download, and its coverage grew with it. The run recorded here covers 46 of the 48 shards: 35 passes, 4 undecided, no failures, with 96,073 of the checkpoint's 96,085 tensors readable from the headers on disk. A separate run against the first 3,000,001 bytes of each of the 48 published shards (roughly 144 MB in total) is the one where all 38 checks pass. **No weight payload has been downloaded or read** in any run.
...
- Upshot: 96,085 tensors, 510,286,023,000 B (475.24 GiB), a fully consistent tensor inventory, an Engram layout that closes to the row, and a checkpoint-specific configuration that differs from the validated V4-Flash config in the ways listed below. The 4 undecided checks are shape claims waiting on the two Engram shards, which are the last and largest — undecided is not a pass, and `--require-complete` turns it into a failure.
...
**There is no V4.1 model.** What exists is a config schema, a host-only audit, an Engram front end that needs no checkpoint at all, an op layer, and an attention stack above that op layer whose parameters are random; the V4-Flash runtime the layer-level comparison below suggests is a partial starting point.
...
That totals 38 checks without an index and 39 with one. A total is not the same as a "pass" count: on the 20 shards downloaded at the first real run, 31 passed and 8 were undecided; on the 46 shards the record now covers, 35 passed, 4 were undecided and none failed, because a check whose evidence is in a shard that has not landed has a third outcome rather than a pass. [Auditing the shards while the checkpoint arrives](../performance/deepseek_v4_1_shard_audit.md) records all three runs and, for each, the waiting checks with the shards each one waits on. The 4 that are still waiting are all Engram shape claims — the table dtype, the tables' 1×32 row block, the gate and value projections on each Engram layer, and the 32×32 block claim that the two `engram.wkv` weights hold open. Everything else resolved as its shard arrived: the CSA2 partition, the per-layer tensor sets, the expert counts, the Engram placement, the vision block set, the `head`/`norm` shapes, the FP4 expert packing and the two DSpark head claims.
...
# the same command later (46 of 48 shards): 35/39, 4 undecided, exit 0
...
- **No generation of any kind.** There is no V4.1 model here, only its attention half: `src/models/deepseek_v4_1/attention.py` runs a stack of CSA2 layers over **random parameters**. No embedding, MoE, Engram lookup, hyper-connection or MTP layer exists, no checkpoint is read, and no token is produced. The tokenizers that have been exercised are V4.1's own and the V4-Flash one, and only to rebuild the Engram compressed vocabulary — not to tokenize a prompt, and not to render a chat template.
- **The attention layers have no oracle either.** The parity and causality properties in [Correctness and precision](#correctness-and-precision) say the layer is internally consistent; they say nothing about whether it matches the released model, and the same TileLang/torch-2.10 wall that blocks the op layer blocks that comparison. Two further gaps are structural rather than environmental: the module runs at one rank, where the reference's cross-rank head split and score all-reduce are the identity, and it exercises 64 hidden channels where the checkpoint has 5,120 — a shape that fits the toy config is not evidence about the released one.
- **Almost all of the checkpoint is local, and none of it is usable here.** The release is 48 shards and 475.24 GiB; 46 of them had landed when the audit last ran — 286.11 GiB covering 96,073 of the 96,085 tensor descriptors — and the two that had not are the Engram tables, 94.56 GiB each. Those two are the only reason 4 checks report undecided rather than passing. Every one of the 96,085 descriptors is still readable as *presence* out of `model.safetensors.index.json`, and the facts that depend on neither come from 48 header prefixes. No weight payload has been read in any run. That 475.24 GiB does not fit on the four 22 GiB cards available here in any case.
...
- [Auditing the shards while the checkpoint arrives](../performance/deepseek_v4_1_shard_audit.md) — the audit runs and how their coverage grew with the download, the per-layer tensor partition, and the shapes the landed shards assert
