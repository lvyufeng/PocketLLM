...
| DeepSeek-V4.1-Flash | 20-layer causal encoder + 20-layer decoder, CSA2 shared-KV attention, Engram, MoE, ViT | Safetensors FP8 E4M3 dense, FP4 E2M1 experts | Host PyTorch over a mapped checkpoint, or the packed fp4 experts on the cards (`--expert-device cuda --expert-world 4`) | Validated on the host CPU: the full text backbone loads from all 48 shards and generates from it; 15 to 42 s per generated token | Not supported: no V4.1 serving path, batching, or MTP; the dense tree stays on the host | [DeepSeek-V4.1-Flash](deepseek-v4.1-flash.md) |
