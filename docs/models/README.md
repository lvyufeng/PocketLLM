...
| MiMo-V2.6-Flash | 9 global-attention + 39 sliding-window layers with a sink, MoE | Safetensors FP8 E4M3 dense, MXFP4 experts, BF16 attention output | Partial: a host reference over the mapped checkpoint, and a heterogeneous routed-expert path that runs on a card out of a host-resident bank | Inspect only: the full backbone decodes on the CPU and the routed experts run on a card, but no request runs end to end on a device | Not implemented | [MiMo-V2.6-Flash](mimo-v2.6-flash.md) |
