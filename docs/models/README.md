...
| MiMo-V2.6-Flash | 9 global-attention + 39 sliding-window layers with a sink, MoE | Safetensors FP8 E4M3 dense, MXFP4 experts, BF16 attention output | `device_model.py` and `ep.py`: the whole 48-layer text backbone out of a host-resident expert bank, one token a step through a KV cache, and the experts dealt out over four ranks | Validated decode on the release: 3.63 tok/s, 275 ms a token on four ranks (1.64 tok/s on one), four ranks byte-identical and the argmax matching the float32 host reference at full depth; no prefill, no batching, attention replicated | Not implemented | [MiMo-V2.6-Flash](mimo-v2.6-flash.md) |
