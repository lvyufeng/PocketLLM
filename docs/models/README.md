...
| MiMo-V2.6-Flash | 9 global-attention + 39 sliding-window layers with a sink, MoE | Safetensors FP8 E4M3 dense, MXFP4 experts, BF16 attention output | `device_model.py`, `device_experts.py` and `ep.py`: the whole 48-layer text backbone out of a host-resident expert bank, a token a step and a prompt a chunk through a KV cache, and the experts dealt out over four ranks | Validated on the release: 3.63 tok/s decode, 275 ms a token on four ranks (1.64 tok/s on one), and 134 tok/s prefill at a 1024-token chunk and 174 at 2048 (46-59x the token-at-a-time loop), four ranks byte-identical and the argmax matching the float32 host reference at full depth; no batching, attention replicated | Not implemented | [MiMo-V2.6-Flash](mimo-v2.6-flash.md) |
