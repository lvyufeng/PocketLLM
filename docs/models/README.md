...
| MiMo-V2.6-Flash | 9 global-attention + 39 sliding-window layers with a sink, MoE | Safetensors FP8 E4M3 dense, MXFP4 experts, BF16 attention output | `device_model.py`: the whole 48-layer text backbone on one card out of a host-resident expert bank, one token a step through a KV cache | Validated decode on the release: 1.52 tok/s, 660 ms a token, the argmax matching the float32 host reference at full depth; no prefill and no batching | Not implemented | [MiMo-V2.6-Flash](mimo-v2.6-flash.md) |
