...
| DeepSeek-V4.1-Flash | 20-layer causal encoder + 20-layer decoder, CSA2 shared-KV attention, Engram, MoE, ViT | Safetensors FP8 E4M3 dense, FP4 E2M1 experts | Inspect only: both released config layouts read into one schema, a 39-check header audit that reports a shard that has not downloaded as undecided rather than as a pass, an Engram hash front end with no GPU consumer, and a CSA2 attention stack that runs over random parameters with no checkpoint loaded | Not supported: no V4.1 model, only its attention half | Not supported: no V4.1 model, only its attention half | [DeepSeek-V4.1-Flash](deepseek-v4.1-flash.md) |
