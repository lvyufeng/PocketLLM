...
| DeepSeek-V4.1-Flash | 20-layer causal encoder + 20-layer decoder, CSA2 shared-KV attention, Engram, MoE, ViT | Safetensors FP8 E4M3 dense, FP4 E2M1 experts | Host PyTorch over a mapped checkpoint; no device path | Validated on the host CPU: the full text backbone loads from all 48 shards and generates from it, with no GPU in the forward; 30 to 40 s per generated token | Not supported: no V4.1 device runtime | [DeepSeek-V4.1-Flash](deepseek-v4.1-flash.md) |
