# Qwen3.8-27B-NVFP4: design and measurements

This is the engineering record behind [the Qwen3.8-27B-NVFP4 model guide](../models/qwen3.8-27b-nvfp4.md):
the checkpoint's mixed quantization scheme, the INT8 kernels that consume it on Turing, the wide-N64
prefill tile, and every measurement behind those choices. Read the model guide first if you want to
*run* the model — this document is for changing it.

The one convention that matters here: NVFP4 is measured **against FP8 on the same hardware and the
same engine binary**, because that is the only comparison that answers the question this format
raises. On SM75 an NVFP4 weight buys memory and costs arithmetic, and both halves of that trade are
stated below.
...
The text architecture is identical to [Qwen3.8-27B-FP8](../models/qwen3.8-27b-fp8.md): 64 text layers (48 Gated DeltaNet + 16 full GQA), hidden 5,120, dense MLP intermediate 17,408, vocabulary 248,320, 24 query heads over 4 KV heads at head dimension 256.
...
- `scripts/bench_qwen_long_context.py`
- [Qwen3.8-27B-FP8](../models/qwen3.8-27b-fp8.md) for the shared text runtime
- [DeepSeek-V4](../models/deepseek-v4.md) for the FP4 INT8 kernels this borrowed scheduling from
