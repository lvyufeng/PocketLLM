# DeepSeek-V4-Flash: design and measurements

This is the engineering record behind [the DeepSeek-V4 model guide](../models/deepseek-v4.md): the
validated geometry, the three execution paths and what each one costs, the precision rules a parity
claim has to respect, and the commands the numbers come from. Read the model guide first if you want
to *run* the model — this document is for changing it.

What this document is not: a single controlled benchmark suite. The rows below come from different
targeted A/B sessions on the same four cards, and a row labelled with an earlier implementation is
kept as the baseline it was measured against rather than folded into one leaderboard.
...
- [GGUF Q2 single-GPU history](../models/deepseek-v4-gguf-q2-single-gpu.md)
