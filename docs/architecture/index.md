...
| [Ternary-Bonsai-2-27B: the reference gate](ternary_bonsai_2_reference_gate.md) | Stage 1's go/no-go: what the upstream ternary reference measures on one 2080 Ti, and the four facts later tasks need — the `PTQ1_0` block, the Hadamard, the one tensor that takes its inverse, and why the kernel is a real integer one. |
| [Xing4.0-29B-A4B: the checkpoint audit](xing4_0_29b_a4b_audit.md) | Stage 2's go/no-go: the `hc_*` matrix hyper-connection written out as a forward pass a reimplementation can be built from, cross-checked against the second published implementation of it; the 977-tensor IQ4_NL inventory; and the four premises in the issue tree that did not survive contact with the artifacts. |
| [Xing4.0-29B-A4B: design and measurements](xing4_0_29b_a4b_design.md) | Stage 5's record behind [the model guide](../models/xing4.0-29b-a4b.md): the residual streams carried wider than the sublayers because this checkpoint's activations leave fp16, the fused hyper-connection kernel and its A-B-A-B 2.17×, the resident MoE with a route sum that is the same twice, the prefill chunk derived from the card's own free memory and the context ceiling that follows from it, where a decode step's 11,536 launches go, and the served request — including the shared-cache bug a served run found. |
