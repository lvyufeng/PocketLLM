# Qwen3.8-27B (official BF16): design and measurements

This is the engineering record behind [the Qwen3.8-27B BF16 model guide](../models/qwen3.8-27b-bf16.md):
the config parsing that reads through the multimodal root, the dense BF16 weight source, the TP4 shard
contract, and what the host-only audit does and does not establish. Read the model guide first if you
want to *run* the model — this document is for changing it.

What this document is not: a performance record. **No on-device generation has been measured for this
checkpoint on the CUDA backend.** Everything below is mapping, sharding and precision policy, verified
on the real 51.7 GiB release with no accelerator present.
...
Generation follows the FP8 page's four-rank NCCL procedure with this checkpoint path substituted; see [Qwen3.8-27B-FP8](../architecture/qwen3_8_27b_fp8_design.md#reproduction).
...
- [Qwen3.8-27B-FP8](../models/qwen3.8-27b-fp8.md) — the validated CUDA runtime this checkpoint reuses
